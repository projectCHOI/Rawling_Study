...
required_numbers_input = input("반드시 포함해야 하는 번호를 입력하세요(공백으로 구분): ")
required_numbers = [int(num) for num in required_numbers_input.split()]

# 사용자로부터 반드시 제외해야 하는 번호를 입력받아 리스트로 변환
exclude_numbers_input = input("반드시 제외해야 하는 번호를 입력하세요(공백으로 구분): ")
exclude_numbers = [int(num) for num in exclude_numbers_input.split()]
